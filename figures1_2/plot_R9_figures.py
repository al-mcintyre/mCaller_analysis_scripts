...
from train_model_201810 import *
import sys
import random
import cPickle
import numpy as np 
...
def extract_data(exp,num,i,qual_thresh,skip_thresh,diffs_by_context,diff_fis=None):
   ...
   for diff_fi in diff_fis:
      for line in open(diff_fi,'r'):
         if len(line.split('\t')) == 8:
            chrom, readname, pos, context, diffs, strand, label, perc = line.split('\t')
         else:
            chrom, readname, pos, context, diffs, strand, label = line.split('\t')   
         read_qual = float(diffs.split(',')[-1].strip())
         if read_qual >= qual_thresh:
            ...
            if len(features) == i and nskips <= skip_thresh:
               ...
               #signals[label.strip()].append(features)
               #contexts[label.strip()].append(context)
               signals.append(features)
               contexts.append(context)
               labels.append(label.strip())
               positions.append(pos)
      print diff_fi, len(signals) #['m6A'])   
   for label in diffs_by_context:
       print len(diffs_by_context[label]),'contexts for',label,':',diffs_by_context[label].keys()[:10]
...
for exp in ['chiu','mason'][1:2]:

   #for qual, qual_threshold in zip(['hq.',''],[9,0]):
   for num,i in zip(nums,num_feats):
      if num == '6.':
         quals,qthreshs = ['','_hq'],[0,9] #orders of qthreshs and sthreahs was reversed 09/27/2018
         skips,sthreshs = ['','_sk'],[0,2]
      else:
         quals,qthreshs = [''],[0]
         skips,sthreshs = [''],[0]
      for qual, qual_threshold in zip(quals,qthreshs):
       for skip, skip_threshold in zip(skips,sthreshs):
         print exp, num, qual_threshold, 'qual', skip_threshold,'skips'
         diffs_by_context = {unmeth:{},meth:{}}
         diffs_by_context,signals,contexts,labels,positions = extract_data(exp,num,i,qual_threshold,skip_threshold,diffs_by_context)
         ...
         #for clf in ['SVM']: #,'SVM']: #RF','SVM','NN','LR','NBC']:
           ...
           if exp == 'chiu':
              ss,cs = {'A':[],'m6A':[]},{'A':[],'m6A':[]}
              for s,l,c in zip(signals,labels,contexts):
                  ss[l].append(s)
                  cs[l].append(c)
              ss_dict = {'general':ss}
              cs_dict = {'general':cs}
              model = model_signals(ss_dict,cs_dict,modelfile='model_'+str(i)+'_'+clf+'_chiu.pkl',classifier=clf,train=True)
              if clf == 'LR':
                  model = model_signals(ss_dict,cs_dict,modelfile='model_'+str(i)+'_'+clf+'_chiu.pkl',classifier=clf,train=True)
              sys.stdout.flush()
           elif exp == 'mason':
              if clf not in prob_scores:
                 prob_scores[clf] = {}
              prob_scores[clf][num],results = model_signals(signals,contexts,modelfile='model_'+str(i)+'_'+clf+'_chiu.pkl',classifier=clf,train=False,labels=labels)
...
fdiffs_by_context, fsignals, fcontexts, flabels, fpositions = extract_data('mason','6.',7,0,0,fdiffs_by_context,diff_fis=['mason_R9_fwd.gm.ecoli.eventalign.diffs.6.fracm6A'])
